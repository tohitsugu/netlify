---
slug: /v1/integration-with-appgate
id: Z869ee80f-e5d0-4e9e-85ef-de2a5153fce5
---
# Integration with Appgate
This document describes the procedure to configure the BlockID Admin Console as a passwordless authentication solution for your organization's Appgate users. This integration will allow your users to log in to  the Appgate application account leveraging their biometrics. The biometric options include TouchID / FaceID and LiveID.
...
1. Admin access to the following:
    ...
    *   Appgate application that supports SAML integration
...
1.	With the above prerequisites, you should now successfully be registered and be able to login to:
    * BlockID Admin Console application. Ensure the email address used to log in to this application is the same as the one used to log in to the Appgate account.
...
1. Configure your BlockID Tenant as an Identity Provider (IDP) within the Appgate application.
2. Configure the Appgate application as a Service Provider (SP) within your BlockID Tenant.

**List of Topics:**

1. [Configure your BlockID Tenant as an Identity Provider (IDP) within the Appgate application](#AppgateIDP)
2. [Configure the Appgate application as a Service Provider (SP) within your BlockID Tenant](#BlockIDIDP)
   a. [Save your organization’s BlockID tenant details](#SaveSAML)
   b. [Set IDP Assertion Claim details](#SetIDPSAML) 
   c. [Add Appgate application as a Service Provider](#SaveSPSAML)
3. [Test the SAML Single Sign-On Connection](#TestSAML)

## <a name = "AppgateIDP"></a>Configure your BlockID Tenant as an Identity Provider (IDP) within Appgate

:::tip Note:
The following steps will be performed by your Appgate administrator. Get your tenant's XML metadata file and SSL certificate from your 1Kosmos representative to configure 1Kosmos as an Identity provider for your company's Appgate application account.
:::

**Steps**

## <a name = "BlockIDIDP"></a>Configure Appgate application as a Service Provider (SP) within your BlockID Tenant
...
2.	From the XML metadata information file, copy and save the values of the following keys to use while performing the Appgate application SAML integration configuration:
...
### <a name = "SaveSPSAML"></a>Add Appgate application as a Service Provider

1.	In the **SAMLv2** screen, click **Service Providers**.
2.	In the **Service Providers List** screen, from the **One-Click Rapid Service Provider Onboarding** section, click on the **Appgate** option.
3.	In the **Onboard Appgate as a SAMLv2 Service Provider** window, enter details for the following fields:
    :::tip Note:
    In case, if your client does not share their Appgate's XML metadata file to 1Kosmos, get the **Entity ID** and **Login URL** (SAML Assertion Consumer Service URL) details of their Appgate application. 
    :::
    * **Application Label**: Enter the appropriate name for your organization’s Appgate application as a service provider.
     * **Entity ID**: Enter the unique entityId URL specified in your single-sign on settings in the Appgate application.
    * **Login URL**: Enter the login URL specified in your single-sign on settings in the Appgate application. 
    * Click **Onboard Appgate**.
4.	Click on the newly added Appgate as a service provider link.
5.	In the **Edit** screen:
    ...
    * In the **SP Service URL End Points** tab:
        * Select `POST`.
        * Ensure ** Appgate** login URL is added.
    * Click **Confirm and Save**.

## <a name = "TestSAML"></a>Test the SAML Single Sign-On Connection

1.	In your browser, enter your organization’s Appgate URL. You will be redirected to the BlockID Admin console’s login screen is displayed with the barcode to be scanned from your BlockID mobile app.
...
6. You will be logged in to your **Appgate** account.
