...
id: Z4c532f57-96a5-4690-9344-5cc15ba3bcad
---
# Release Notes
...
The platform can now accept RADIUS authentication requests from clients and enable secure logon with addition OTP

**BlockID Broker**
Enables remote authentication with on-premise directories without requiring site-site VPN
...
**User Management**
BlockID now supports user management with full scale RBAC enabling administrators to control granular access to the admin console

**Enterprise Windows Logon**

Ability to logon to windows workstation using BlockID. A credential provider needs to be available on the windows workstation to support this. A sample video of this feature is available [here](https://www.youtube.com/watch?v=U8EapZIPDw4) 

**Push Notification**

BlockID now supports the ability to send device push notifications when the user initiates a logon process. This will be available along with the ability for a user to scan the QR code

**Password Reset**

BlockID will support the ability for a user to reset their enterprise Active Directory password through the BlockID Enterprise App
