---
slug: /blockid-ios-sdk-141and142
id: blockid-ios-sdk-141and142
---
# BlockID iOS SDK 1.4.1 and 1.4.2

:::tip Warning:
This document is compatible with iOS SDK v1.4.1 and v1.4.2.
:::
...
![BIDFramework.jpg](https://cdn.document360.io/aeae7c88-d2f4-4a9c-bba9-c4f368260d78/Images/Documentation/BIDFramework.jpg)

2.	Add the `BlockIDSDK.framework` folder into the Frameworks section.
![Frameworksfolder.jpg](https://cdn.document360.io/aeae7c88-d2f4-4a9c-bba9-c4f368260d78/Images/Documentation/Frameworksfolder.jpg)
...
![EmbedandSign.jpg](https://cdn.document360.io/aeae7c88-d2f4-4a9c-bba9-c4f368260d78/Images/Documentation/EmbedandSign.jpg)
4. Navigate to ***Build Settings > Validate Workspace > Yes***.
5. The following script is to remove the non required architecture while creating the `iOS App Store Package (IPA)` file:
```swift
...
```
5. Update the scheme option from `Debug` to `Release`.
...
* To scan driver's licenses, the app should call the `DriverLicnseScanHelper` class. This scanner scans the front and backside of the driver's license and handles the UI constraints of back and front screens. 
...
```
Use the following code to start e-Chip (RFID) scanning for passport:
...
| isWithRFID | Type of Boolean. It returns `true` if the RFID chip of the passport is scanned or getting scanned, else returns `false`. |
...
**Enroll NationalID**
Perform the following steps to enroll the nationalID:

* To scan the NationalID, the app should call the `NationalIDScanHelper` class. This scanner scans the front and backside of the NationalID. 
* There are parameters for the scanning side of the NationalID. Based on this parameter the scanner will act and scan the document accordingly. 
* The front side of the document will be scanned using `OCR`, and the backside of the document will be used to scan `OCR`, `MRZ`, and `QRCode`.
* Check the following method that presents the NationalIdScanHelper class:
...
private let firstScanningDocSide: NIDScanningSide = .NATIONAL_ID_BACK
 // this parameter will be .NATIONAL_ID_BACK or .NATIONAL_ID_FRONT

//initialize the scanner object
nationalIDScanHelper = NationalIDScanHelper.init(scanningMode: selectedMode, bidScannerView: _viewLiveIDScan, nidScanResponseDelegate
...
Use the following code to start NationalID scanning:

```swift
self.nationalIDScanHelper?.startNationalIDScanning(scanningSide: firstScanningDocSide)
```

Request Params Description: 
| Params | Description |
| ----------- | ----------- |
| scanningSide | Type of `NIDScanningSide`. The values of the above parameters are `.NATIONAL_ID_BACK` and `.NATIONAL_ID_FRONT`.|
...
Use the following method when the NationalID is ready for detection:
...
**scanBackSide**
Use the following method when the NationalID is ready for back side scan:

```swift
func scanBackSide(){
 //your code here
   self.nationalIDScannerHelper?.startNationalIDScanning(scanningSide: .NATIONAL_ID_BACK)
} 
```

**scanFrontSide**
Use the following method when the NationalID is ready for  front side scan:

```swift
func scanFrontSide(){
   //your code here
   self.nationalIDScannerHelper?.startNationalIDScanning(scanningSide: .NATIONAL_ID_FRONT)
}
```

**nidScanCompleted**
Use the following method when the NationalID scanning is completed:

```swift
func nidScanCompleted(nidScanSide: NIDScanningSide, bidNationalID: BIDNationalID?, signatureToken signToken: String?, error: ErrorResponse?) { //your code here
}
```
...
| Params | Description |
| ----------- | ----------- |
| nidScanSide | Type of `NIDScanningSide`. The values of the above parameters are `.NATIONAL_ID_BACK` and `.NATIONAL_ID_FRONT`.|
| bidNationaID | Type of `BIDNationalID`. It returns the object of the scanned document. |
| signatureToken | Type of string. The MD5 value of BIDNationalID object. |
| error | Type of `ErrorResponse`. It returns the errorObject if the NationalID scanning is not completed and fails with an error. |

***Save NationalID***
To save a NationalID object, call the BlockID SDK  `registerDocument` method:

```swift
BlockIDSDK.sharedInstance.registerDocument(obj: nidObject, docType: .nationalId, sigToken: token){status, error in
      //your code here
}
```
...
***Check if the NationalID is Enrolled***
Use the following method to check if the NationalID is enrolled/registered with the BlockID:
```swift
BlockIDSDK.sharedInstance.isNationalIDEnrolled()
```
***Unenroll NationalID***
Use the BlockID SDK unregisterDocument method to unenroll a NationalID:
```swift
BlockIDSDK.sharedInstance.unregisterDocument(docType: .nationalId) {
                status, error in
}
```
...
|  Params  |                                                                                                                                                                                    Description                                                                                                                                                                                    |
|:--------:|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|
| status   | Boolean value: True / false True: on success False: on failure                                                                                                                                                                                                                                                                                                                    |
| error    | NSObject of ErrorResponse. The above object consists of two parameters: Code type of integer The message type of string                                                                                                                                                                                                                                                           |
| response | Type of AccessCodeValidationResponseDecryptedData var id: String! var uuid: String! var type: String! var tenantId: String! var communityId: String! var accesscodepayload: AccessCodeResponseDataPayload! var status: String! public class AccessCodeResponseDataPayload: Codable { var userid: String! var authType: String! var invite_email: String! var otp_email: String! } |
...
```swift
BlockIDSDK.sharedInstance.redeemAccessCode(code: magicLinkDataCode, userId: userid, password: password, origin: bidOrigin, deviceToken: apnsDeviceToken, lat: locationLatitude, lon: locationLogitude) { (status, error) in
    //your code here
}
```
...
```swift
BlockIDSDK.sharedInstance.linkUser(userId: userId, origin: origin, deviceToken: apnstoken, lon: locationLongitude, lat:locationLatitude) { (status, error) in
    // your code here
}
```
...
```swift
BlockIDSDK.sharedInstance.addNativeAccount(method: method, userId: userid, password: password, origin: bidOrigin!, deviceToken: devicetoken, lon: locationLongitude, lat:locationLatitude) { (status, error) in
    // your code here
}
```
Request Params Description: 
...
```swift
BlockIDSDK.sharedInstance.authenticateUser(sessionId: datamodel.session, creds: datamodel.creds, scopes: datamodel.scopes, lat: locationLatitude, lon: locationLongitude, origin: datamodel.origin, userId: datamodel.userId) { (status, error) in
...
| kDocumentPhotoComparisionFailed | 411 | Your photo on the document does not match with LiveID |
...
| kPassportNationalIDNotMatch | 419 | Passport and NationalID data doesn't matched. |
| kNationalIdDLNotMatch | 420 | NationalID and Driver license data doesn't matched. |
| kPPAboutToExpire | 100001 | Passport will expire within 90 days, Would you like to continue enrollment? |
| kPPExpired | 100002 | Expired passport. |
| kInvalidPP | 100003 | Invalid passport |
| kInvalidPPBioData | 100004 | Passport Bio-Data not scanned properly |
| kInvalidPPEChipData | 100005 | Invalid E-Passport Chip Data |
| kPPNotMatchWithDL | 100006 | Passport and Driver license data does not match. |
| kPPNotMatchWithNID | 100007 | Passport and National ID data does not match. |
| kPPFaceNotMatchWithLiveID | 100008 | Your passport photo does not match with profile photo.|
