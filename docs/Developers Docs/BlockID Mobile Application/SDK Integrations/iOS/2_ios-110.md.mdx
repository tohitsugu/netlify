...
4. Copy `OpenSSL.framework` into your application. and add it into the Frameworks section.
5. In the project editor, navigate to ***General > Library > Frameworks > Frameworks, Libraries, and Embedded Content***, select `Embed & Sign` and add the `libcrypto.a`and `libssl.a` files into the ***Build Phase > Link Binary With Libraries*** section.
