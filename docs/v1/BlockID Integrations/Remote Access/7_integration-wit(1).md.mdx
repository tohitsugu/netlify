---
slug: /v1/integration-with-vmware
id: Za4b469f7-7ac3-45f9-8e46-75267cf6a1a8
---
# Integration with VMware
This document describes the procedure to configure the BlockID Admin Console as a passwordless authentication solution for your organization's VMware users. This integration will allow your users to log in to  the VMware application account leveraging their biometrics. The biometric options include TouchID / FaceID and LiveID.
...
1. Admin access to the following:
    ...
    *   VMware application that supports SAML integration
...
1.	With the above prerequisites, you should now successfully be registered and be able to login to:
    * BlockID Admin Console application. Ensure the email address used to log in to this application is the same as the one used to log in to the VMware account.
...
1. Configure your BlockID Tenant as an Identity Provider (IDP) within the VMware application.
2. Configure the VMware application as a Service Provider (SP) within your BlockID Tenant.

**List of Topics:**

1. [Configure your BlockID Tenant as an Identity Provider (IDP) within the VMware application](#VMwareIDP)
2. [Configure the VMware application as a Service Provider (SP) within your BlockID Tenant](#BlockIDIDP)
   a. [Save your organization’s BlockID tenant details](#SaveSAML)
   b. [Set IDP Assertion Claim details](#SetIDPSAML) 
   c. [Add VMware application as a Service Provider](#SaveSPSAML)
3. [Test the SAML Single Sign-On Connection](#TestSAML)

## <a name = "VMwareIDP"></a>Configure your BlockID Tenant as an Identity Provider (IDP) within VMware

:::(Warning) (Note:)
The following steps will be performed by your VMware administrator. Get your tenant's XML metadata file and SSL certificate from your 1Kosmos representative to configure 1Kosmos as an Identity provider for your company's VMware application account.
:::

**Steps**

## <a name = "BlockIDIDP"></a>Configure VMware application as a Service Provider (SP) within your BlockID Tenant
...
2.	From the XML metadata information file, copy and save the values of the following keys to use while performing the VMware application SAML integration configuration:
...
### <a name = "SaveSPSAML"></a>Add VMware application as a Service Provider

1.	In the **SAMLv2** screen, click **Service Providers**.
2.	In the **Service Providers List** screen, from the **One-Click Rapid Service Provider Onboarding** section, click on the **VMware** option.
3.	In the **Onboard VMware as a SAMLv2 Service Provider** window, enter details for the following fields:
    :::(Info) (Note:)
    In case, if your client does not share their VMware's XML metadata file to 1Kosmos, get the **Entity ID** and **Login URL** (SAML Assertion Consumer Service URL) details of their VMware application. 
    :::
    * **Application Label**: Enter the appropriate name for your organization’s VMware application as a service provider.
     * **Entity ID**: Enter the unique entityId URL specified in your single-sign on settings in the VMware application.
    * **Login URL**: Enter the login URL specified in your single-sign on settings in the VMware application. 
    * Click **Onboard VMware**.
4.	Click on the newly added VMware as a service provider link.
5.	In the **Edit** screen:
    ...
    * In the **SP Service URL End Points** tab:
        * Select `POST`.
        * Ensure ** VMware** login URL is added.
    * Click **Confirm and Save**.

## <a name = "TestSAML"></a>Test the SAML Single Sign-On Connection

1.	In your browser, enter your organization’s VMware URL. You will be redirected to the BlockID Admin console’s login screen is displayed with the barcode to be scanned from your BlockID mobile app.
...
6. You will be logged in to your **VMware** account.
