...
Verify the information on a user's driver's license or state identification card with the issuing department of motor vehicles.  

## **State Driver's License Verification**

To verify the information on a user's driver's license you will need to provide the information as shown on the driver's license, such as the license number, the license expiration date, and all other user details. 
...
```jsx title="200 Success"
{
  ...
  "result": {
    ...
    "middle_name_exact_match": true,
    "middle_name_fuzzy_match": true,
...
```
The validation request was unsuccessful due to validation errors
